...
struct wake_lock tp_hx_update_wakelock;
/* Huaqin add for nvt fw update fail by limengxia at 20181113 end */
...
UPDATE_FW:
	/* Huaqin add for nvt fw update fail by limengxia at 20181113 start */
	wake_lock_init(&tp_hx_update_wakelock, WAKE_LOCK_SUSPEND, "tp-update");
	wake_lock(&tp_hx_update_wakelock);
	tp_fw_update_flag = 1;
	/* Huaqin add for nvt fw update fail by limengxia at 20181113 end */
	if (i_update_FW() <= 0) {
	...
	/* Huaqin add for nvt fw update fail by limengxia at 20181113 start */
	tp_fw_update_flag = 0;
	wake_unlock(&tp_hx_update_wakelock);
	/* Huaqin add for nvt fw update fail by limengxia at 20181113 end */
}
#endif
...
int himax_chip_common_init(void)
{
	...
	/* Huaqin modify for ZQL1820-838 by limengxia at 2018/10/11 end */
	int i = 0, ret = 0, err = -1;
	/* Huaqin modify for ZQL1820-838 by limengxia at 2018/10/11 end */
	struct himax_ts_data *ts = private_ts;
	struct himax_i2c_platform_data *pdata;
